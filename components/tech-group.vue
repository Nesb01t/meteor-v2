<script setup lang="ts"></script>

<template>
  <div class="tech-group">
    <div class="scroll-container">
      <div class="scroll-content left-scroll">
        <img src="https://skillicons.dev/icons?i=threejs" alt="threejs" />
        <img src="https://skillicons.dev/icons?i=nextjs" alt="nextjs" />
        <img src="https://skillicons.dev/icons?i=solidjs" alt="solidjs" />
        <img src="https://skillicons.dev/icons?i=tailwind" alt="tailwind" />
        <img src="https://skillicons.dev/icons?i=nuxtjs" alt="nuxtjs" />
        <img src="https://skillicons.dev/icons?i=d3" alt="d3" />
        <img src="https://skillicons.dev/icons?i=css" alt="css" />
        <img src="https://skillicons.dev/icons?i=html" alt="html" />
        <img src="https://skillicons.dev/icons?i=sass" alt="sass" />
        <img src="https://skillicons.dev/icons?i=astro" alt="Astro" />
        <img src="https://skillicons.dev/icons?i=express" alt="express" />
        <img src="https://skillicons.dev/icons?i=spring" alt="spring" />
        <img src="https://skillicons.dev/icons?i=ktor" alt="ktor" />
        <img src="https://skillicons.dev/icons?i=nestjs" alt="nestjs" />
        <img src="https://skillicons.dev/icons?i=prisma" alt="prisma" />
        <img src="https://skillicons.dev/icons?i=dart" alt="dart" />
        <img src="https://skillicons.dev/icons?i=kotlin" alt="Kotlin" />
        <img src="https://skillicons.dev/icons?i=elixir" alt="elixir" />
        <img src="https://skillicons.dev/icons?i=java" alt="java" />
        <img src="https://skillicons.dev/icons?i=nodejs" alt="nodejs" />
        <img src="https://skillicons.dev/icons?i=py" alt="py" />
        <img src="https://skillicons.dev/icons?i=cs" alt="cs" />
        <img src="https://skillicons.dev/icons?i=cpp" alt="cpp" />
        <img src="https://skillicons.dev/icons?i=typescript" alt="TypeScript" />
        <img src="https://skillicons.dev/icons?i=lua" alt="Lua" />
        <img src="https://skillicons.dev/icons?i=unreal" alt="unreal" />
        <img src="https://skillicons.dev/icons?i=unity" alt="Unity" />
        <img src="https://skillicons.dev/icons?i=electron" alt="electron" />
        <img src="https://skillicons.dev/icons?i=flutter" alt="flutter" />
        <img src="https://skillicons.dev/icons?i=godot" alt="godot" />
        <img src="https://skillicons.dev/icons?i=tauri" alt="tauri" />
        <img src="https://skillicons.dev/icons?i=qt" alt="qt" />
        <img src="https://skillicons.dev/icons?i=dotnet" alt="DotNet" />
        <img src="https://skillicons.dev/icons?i=vercel" alt="vercel" />
        <img src="https://skillicons.dev/icons?i=gradle" alt="gradle" />
        <img
          src="https://skillicons.dev/icons?i=githubactions"
          alt="githubactions"
        />
        <img src="https://skillicons.dev/icons?i=figma" alt="figma" />
        <img src="https://skillicons.dev/icons?i=rabbitmq" alt="rabbitmq" />
        <img src="https://skillicons.dev/icons?i=jenkins" alt="jenkins" />
        <img src="https://skillicons.dev/icons?i=redis" alt="redis" />
        <img src="https://skillicons.dev/icons?i=sqlite" alt="sqlite" />
        <img src="https://skillicons.dev/icons?i=docker" alt="Docker" />
        <img src="https://skillicons.dev/icons?i=nginx" alt="nginx" />
        <img src="https://skillicons.dev/icons?i=git" alt="git" />
        <img src="https://skillicons.dev/icons?i=blender" alt="blender" />
        <img src="https://skillicons.dev/icons?i=notion" alt="Notion" />
        <img src="https://skillicons.dev/icons?i=markdown" alt="Markdown" />
        <img src="https://skillicons.dev/icons?i=svg" alt="svg" />
      </div>

      <div class="scroll-content right-scroll">
        <img src="https://skillicons.dev/icons?i=threejs" alt="threejs" />
        <img src="https://skillicons.dev/icons?i=nextjs" alt="nextjs" />
        <img src="https://skillicons.dev/icons?i=solidjs" alt="solidjs" />
        <img src="https://skillicons.dev/icons?i=tailwind" alt="tailwind" />
        <img src="https://skillicons.dev/icons?i=nuxtjs" alt="nuxtjs" />
        <img src="https://skillicons.dev/icons?i=d3" alt="d3" />
        <img src="https://skillicons.dev/icons?i=css" alt="css" />
        <img src="https://skillicons.dev/icons?i=html" alt="html" />
        <img src="https://skillicons.dev/icons?i=sass" alt="sass" />
        <img src="https://skillicons.dev/icons?i=astro" alt="Astro" />
        <img src="https://skillicons.dev/icons?i=express" alt="express" />
        <img src="https://skillicons.dev/icons?i=spring" alt="spring" />
        <img src="https://skillicons.dev/icons?i=ktor" alt="ktor" />
        <img src="https://skillicons.dev/icons?i=nestjs" alt="nestjs" />
        <img src="https://skillicons.dev/icons?i=prisma" alt="prisma" />
        <img src="https://skillicons.dev/icons?i=dart" alt="dart" />
        <img src="https://skillicons.dev/icons?i=kotlin" alt="Kotlin" />
        <img src="https://skillicons.dev/icons?i=elixir" alt="elixir" />
        <img src="https://skillicons.dev/icons?i=java" alt="java" />
        <img src="https://skillicons.dev/icons?i=nodejs" alt="nodejs" />
        <img src="https://skillicons.dev/icons?i=py" alt="py" />
        <img src="https://skillicons.dev/icons?i=cs" alt="cs" />
        <img src="https://skillicons.dev/icons?i=cpp" alt="cpp" />
        <img src="https://skillicons.dev/icons?i=typescript" alt="TypeScript" />
        <img src="https://skillicons.dev/icons?i=lua" alt="Lua" />
        <img src="https://skillicons.dev/icons?i=unreal" alt="unreal" />
        <img src="https://skillicons.dev/icons?i=unity" alt="Unity" />
        <img src="https://skillicons.dev/icons?i=electron" alt="electron" />
        <img src="https://skillicons.dev/icons?i=flutter" alt="flutter" />
        <img src="https://skillicons.dev/icons?i=godot" alt="godot" />
        <img src="https://skillicons.dev/icons?i=tauri" alt="tauri" />
        <img src="https://skillicons.dev/icons?i=qt" alt="qt" />
        <img src="https://skillicons.dev/icons?i=dotnet" alt="DotNet" />
        <img src="https://skillicons.dev/icons?i=vercel" alt="vercel" />
        <img src="https://skillicons.dev/icons?i=gradle" alt="gradle" />
        <img
          src="https://skillicons.dev/icons?i=githubactions"
          alt="githubactions"
        />
        <img src="https://skillicons.dev/icons?i=figma" alt="figma" />
        <img src="https://skillicons.dev/icons?i=rabbitmq" alt="rabbitmq" />
        <img src="https://skillicons.dev/icons?i=jenkins" alt="jenkins" />
        <img src="https://skillicons.dev/icons?i=redis" alt="redis" />
        <img src="https://skillicons.dev/icons?i=sqlite" alt="sqlite" />
        <img src="https://skillicons.dev/icons?i=docker" alt="Docker" />
        <img src="https://skillicons.dev/icons?i=nginx" alt="nginx" />
        <img src="https://skillicons.dev/icons?i=git" alt="git" />
        <img src="https://skillicons.dev/icons?i=blender" alt="blender" />
        <img src="https://skillicons.dev/icons?i=notion" alt="Notion" />
        <img src="https://skillicons.dev/icons?i=markdown" alt="Markdown" />
        <img src="https://skillicons.dev/icons?i=svg" alt="svg" />
      </div>

      <div
        class="absolute top-0 left-0 bg-gradient-to-r transition-all dark:from-black/95 from-white to-transparent w-[7%] h-full z-10"
      ></div>

      <div
        class="absolute top-0 right-0 bg-gradient-to-l transition-all dark:from-black/95 from-white to-transparent w-[7%] h-full z-10"
      ></div>
    </div>
  </div>
</template>

<style lang="scss" scoped>
.tech-group {
  @apply max-w-[1200px] overflow-hidden opacity-85;

  .scroll-container {
    @apply flex relative overflow-hidden w-full min-w-[930px] max-w-[930px];
    overflow: hidden;
    white-space: nowrap;
  }

  .scroll-content {
    @apply flex flex-col h-[180px] w-full min-w-[930px];
    flex-wrap: wrap;
  }

  .left-scroll {
    animation: infinite-scroll 15s linear infinite;
  }

  .right-scroll {
    animation: infinite-scroll-placeholder 15s linear infinite;
  }

  .scroll-content img {
    margin: 5px;
  }
}

@keyframes infinite-scroll {
  0% {
    transform: translateX(0);
  }
  100% {
    transform: translateX(-100%);
  }
}

@keyframes infinite-scroll-placeholder {
  0% {
    transform: translateX(0);
  }
  100% {
    transform: translateX(-100%);
  }
}
</style>
